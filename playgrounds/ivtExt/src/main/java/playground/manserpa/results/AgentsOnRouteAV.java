...
/**
 * 
 * Purpose: Show the number of agents traveling with A-Taxis at a given time of the day
 * 
 * Input: shape file, network.xml, events.xml
 * Output: csv with the time of the day and the number of agents traveling with A-Taxis
 * 
 * @author manserpa
 * 
 */
...
public final class AgentsOnRouteAV {
	...
	public static void main(String[] args) throws IOException	{
		AgentsOnRouteAV cs = new AgentsOnRouteAV(args[0]);
		
		cs.run(args[1], args[2]);
		
	}
	
	private AgentsOnRouteAV(String shpFile)	{
	...
	private void run(String networkFile, String eventFile) throws IOException	{
	    ...
	    String csvFilewriter = "AgentsOnRouteAV.csv";
	    FileWriter writer = new FileWriter(csvFilewriter );
	    
	    CSVUtils.writeLine(writer , Arrays.asList("TimeSlice","AVs"), ';');
				...
				public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException	{
					...
					if(qName.equalsIgnoreCase("event"))	{
						...
						if(attributes.getValue("type").equals("departure"))	{
							...
							if(attributes.getValue("legMode").equals("av"))	{
